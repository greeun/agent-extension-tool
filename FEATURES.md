...
- **공통(모든 서브탭)**: `Space` = 포커스 항목 마크/해제 (좌측 체크박스 ■/□). 마크가 있으면 `p`/`g`가 마크된 전체 항목에 일괄 적용, `u`가 마크된 전체 항목을 일괄 업데이트 (둘 다 확인 모달; `u`는 항목별 실패가 나머지를 중단시키지 않고 상태바에 `N updated, N up to date, N failed` 집계 표시), `Esc`가 마크 해제 → 검색 해제 → 포커스 상승 순으로 동작
