...
# ── Section 11: TUI Common Helpers — moved to axt/tui/widgets.py ────────────
# ── Section 12: TUI Common Widgets — moved to axt/tui/widgets.py ────────────
#
# Section 13 below still needs the helpers/widgets above (CP_SEL, render_table,
# etc.). They land on this module's globals via the package-level mirror in
# axt/__init__.py — axt.tui.widgets is imported first, then axt._core, and
# both halves are mirrored onto the `axt` package. To make the legacy bare-name
# references in Section 13 work (e.g. `render_table(...)` instead of
# `widgets.render_table(...)`), we re-import the names below.
from axt.tui.widgets import *  # noqa: F401,F403 — see C3 plan
from axt.tui.widgets import (  # noqa: F401 — pull `_`-prefixed names
    _draw_cell,
    _safe_pair,
    _wrap_to_cells,
)
